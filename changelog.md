...
### Architecture
- The active chain separates upstream provenance, inherited baseline reproduction, review-contract adoption, runtime-mode adoption, packaging/security/accessibility, and later local product changes.
...
- No portable-trust adapter, backend, private device record, approval action, capability, cache integration, runtime-mode policy enforcement, or QSO-STUDIO contract implementation was added.
...
### Release
- Release remains blocked until mirror/fork/derivative status, naming, attribution, supported platform, tests/build, security, review-contract ownership, runtime-mode policies, gluing fixtures, SBOM, checksums, provenance, and rollback are approved and verified.
- The static console is not an AionUi application release and does not satisfy desktop, WebUI, adapter, or portable-trust acceptance gates.
...
- 2026-07-21 — Exact-head validation for the ADR/topology candidate remains pending.

### Deployment
- No public binary, update channel, privileged adapter, remote administration path, remote WebUI, or portable-trust backend is authorized before the fork, review-role, runtime-mode, security, and distribution decisions.
...
- The least-privilege Pages validation workflow validates static documentation and console assets; deployment remains dependent on repository Pages settings, review, and branch merge.
